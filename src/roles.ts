...
function place(e: Entity, addr: string | null, yOffset: number): void {
  const pos = addr ? playerWorldPos(addr) : null
  if (!pos) {
...
function rolesSystem(dt: number): void {
  ...
  place(firekeeper, fk, 2.5)
  // If one person holds both, stack the second title higher.
  place(woodcutter, wc, fk && wc === fk ? 3.1 : 2.5)
}
